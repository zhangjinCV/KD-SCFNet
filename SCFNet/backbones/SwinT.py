...
class SwinTransformer(nn.Layer):
    ...
    def forward_features(self, x):
        ...
        for layer in self.layers:
            ...
            y4 = x
        bs = y1.shape[0]
        y1 = paddle.reshape(y1, (bs, 96, 96, 128))
        y2 = paddle.reshape(y2, (bs, 48, 48, 256))
        y3 = paddle.reshape(y3, (bs, 24, 24, 512))
        y4 = paddle.reshape(y4, (bs, 12, 12, 1024))
    ...
    def forward(self, x):
        y1, y1, y2, y3, y4 = self.forward_features(x)
        return y1, y1, y2, y3, y4
